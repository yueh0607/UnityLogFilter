...
namespace FFramework.FUnityEditor
{
    ...
    public static class HideLog
    {
        ...
        {
            bool exist = false;
            foreach (var s in LogFilterSettings.instance.BuiltInInfos)
            {
                if (exist) break;
                exist |= str.Contains(s);
            }
            foreach (var s in LogFilterSettings.instance.CustomInfos)
            {
                if (exist) break;
                exist |= str.Contains(s);
            }
    ...
    public class LoggerFilter : EditorWindow
    {
        ...
        [MenuItem("LoggerFilter", menuItem = "FFramework/LogFilter")]
                ...
                case 0:
                    {
                        if (GUILayout.Button("Add", GUILayout.ExpandWidth(false)))
                            AddTo(ref LogFilterSettings.instance.BuiltInInfos, add);
                        GUILayout.EndHorizontal();

                        DrawList(ref LogFilterSettings.instance.BuiltInInfos, ref POS);
                        break;
                    }
                case 1:
                    {
                        if (GUILayout.Button("Add", GUILayout.ExpandWidth(false)))
                            AddTo(ref LogFilterSettings.instance.CustomInfos, add);

                        GUILayout.EndHorizontal();
                        DrawList(ref LogFilterSettings.instance.CustomInfos, ref POS2);
                        break;
                    }
            }
        ...
        {
            LogFilterSettings.instance.Save();
        }
